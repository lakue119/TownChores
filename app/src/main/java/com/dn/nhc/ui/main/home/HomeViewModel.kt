...
class HomeViewModel @Inject constructor(
    ...
    fun getPheedList(size: Int, page: Int, lat: Double, lnt: Double){
        fetchPheedList(size, page, lat, lnt)
    }

    private fun fetchPheedList(size: Int, page: Int, lat: Double, lnt: Double){
        viewModelScope.launch {
            val response = pheedRemoteDataSource.getPheedList(size, page, lat, lnt)
